	...
	public void trade()  {
		...
		while (it.hasNext()){
			...
			if(mSymbol != stockSymbol.getStockSymbol()){
				...
				iterateMethod(buyOrdersArray, sellOrdersArray, buyMap, sellMap, buy, sell, maxNum, maxPrice);


				priceSetter.setNewPrice(m, buyOrdersArray.get(0).getStockSymbol(), maxPrice);
				
				try{
					tradeForBuyOrder(buyOrdersArray);
					tradeForSellOrder(sellOrdersArray);
  	...
  	public void tradeForSellOrder(ArrayList<Order> sellOrdersArray) {
  	...
  	public void tradeForBuyOrder(ArrayList<Order> buyOrdersArray) {
    		for (int k = 0;  k < sellOrdersArray.size(); k ++){
  	...
  	public void iterateMethod(ArrayList<Order> buyOrdersArray,
				ArrayList<Order> sellOrdersArray,
				HashMap<Double, Double> buyMap,
				HashMap<Double, Double> sellMap,
